...
- **routing**: 明示されたtool `cwd` / `workdir`をtaskのcanonical cwdから解決し、なければtask cwdをeffective baseとする。tool schemaのpath field、patch header、literalなfile operand、shellの順序付きliteral `cd`、`git -C`の実operandを抽出し、relative pathをeffective baseから解決する。`.` / `..`とsymlinkを解決し、存在しない末尾は最長の存在ancestorをcanonicalizeしてからpath separator境界を保って正規化する。解決済みpathまたはrepository identityが対象repo rootと同一か配下の場合だけtarget-relevantとする。`cwd` / `workdir` keyや`git -C` tokenの存在だけではtarget-relevantにしない。全operandが明確に別repositoryならtarget-irrelevant、変数展開、subshell、動的path、複数の曖昧なcwd遷移、schema不明、canonicalize失敗などで対象可能性を排除できなければ`unscoped`としてfail closedにする。
- **provenance**: defaultは`task-owned`とする。現在のguard runがfinal initial read直後に発行した許可済みpreflight / fallback snapshot invocationだけは、発行順、tool名、canonical arguments、run-local boundaryをledgerへ記録し、post-readで一意に対応したexact item IDを`guard-owned`とする。同じcommand文字列という理由だけで除外しない。対応が欠落、重複、partial、または一意でなければfail closedにする。`guard-owned` itemはscope anchor / fingerprintへ入れず、Git snapshot evidenceとして別管理する。
- **effect / lifecycle**: tool schemaとcommand全体がfilesystem write、process起動、background / scheduled actionを行わないと保証できる場合だけ`read-only`とし、それ以外は`mutating-or-unknown`とする。`mutating-or-unknown`なtarget-relevant task-owned invocationがpending / running / terminal不明、または子孫・background actionの終了を確認できない場合はtaskを`unscoped`として開始をblockする。terminal化を確認した後に新しいGit snapshotでguard全体をやり直す。lifecycle metadataが取得不能、redacted、partial、truncatedならunfinishedとしてfail closedにする。tool output / result本文はscope evidenceにしない。
...
5. 全Codex taskを同じ引数とscope復元規則で再取得してfinal initial read passを作る。最初のpassとtask ID、`hostId`、canonical identity、`cwd`、live status、scope anchor chain、scope fingerprint、final classificationが一致し、unfinishedなtarget-relevant `mutating-or-unknown` invocationがなく、全callが完全なら、callerのlatest item boundaryと空のguard-owned ledgerを記録する。最後の`read_thread`の次のtool callとしてGit snapshot取得を開始する。差分または失敗があればGit snapshotを取らず、Workflow 9と同じ全体retry制限でguardを最初からやり直す。
