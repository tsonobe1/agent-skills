...
- **provenance**: defaultは`task-owned`とする。guard開始時、最初のtool callより前にcaller item boundary、versioned run ID、空ledgerを固定する。現在のguard runがWorkflow 1–9で発行した許可済みevidence collection invocationは、purpose、effect、lifecycle、発行順、tool名、canonical arguments、run boundaryをledgerへ記録し、各read passで一意に対応したexact item IDを`guard-owned`とする。対象はrepo root / identity、script path lookup、task inventory / scope read、preflight、fallback snapshot、post-read identity probeに限定する。mutationを許すのは、authoritative baselineをfreshにするためWorkflowで明示したbounded ref refreshだけとし、terminal完了、固定したref / SHA、観測したeffectが契約どおりであることをGit snapshotで検証する。その他のmutation、unfinished / unknown lifecycle、想定外effectはfail closedにする。同じcommand文字列という理由だけで除外しない。対応が欠落、重複、partial、または一意でなければfail closedにする。`guard-owned` itemはscope anchor / fingerprintへ入れず、Git / task / identity evidenceとして別管理する。
  ...
  完了・停止を問わず各guard runの結果を返す直前に、stableなassistant message itemとして`guard provenance record`を1個残す。recordは`payload`と`recordHash`をsibling fieldとして持つ。`payload`はschema version、run ID、caller task ID / `hostId`、開始 / 終了item boundary、target local / hosted repository identity、terminal resultを持ち、ledger各項目を発行順のexact item ID、purpose、tool名、canonical argumentsのSHA-256、解決済みrouting、effect、lifecycleで表す。`recordHash`は自身を含めず`payload`だけをRFC 8785 JSON Canonicalization SchemeでserializeしたUTF-8 bytes（BOMなし、前後whitespace / trailing newlineなし）のSHA-256 lowercase hexadecimalとする。duplicate key、non-finite number、RFC 8785で表現不能な値はrecord作成・検証をfail closedにする。後続runは必要なfull historyをcursorで取得し、`payload`から再計算した`recordHash`、boundary、terminal result、各exact item、tool名、arguments hash、routing、effect、lifecycleを実履歴と完全照合できた場合だけ、その項目をhistorical `guard-owned`として継承する。prose、command文字列の類似、record単独、partial / redacted / truncated history、hash / boundary / item不一致では継承しない。target-relevantな旧guard候補を完全検証できない場合は`task-owned`と断定してphantom ownershipを作らず`unscoped`としてfail closedにする。検証済みhistorical `guard-owned` itemもscope anchor / fingerprintへ入れない。
...
1. 最初のtool callより前にcaller item boundaryと空のguard-owned ledgerを固定する。repo root、local repository identity、authoritative hosted identity bindingだけを解決し、発行したexact invocationをpurpose付きでledgerへ追加する。repo instructions指定または一意なremote URL以外からhosted identityを推測しない。この時点ではbranch、worktree一覧、HEAD、dirty state、diffを個別Git commandで収集しない。
