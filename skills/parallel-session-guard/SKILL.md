...
target repositoryのlocal identityは常に必須とする。authoritative hosted identityは、repo instructionsが明示的に要求するか、復元したscope / tool invocation / 新しい予定scopeにstructured repository selectorが現れた場合だけ`required`として遅延解決し、それ以外は`not-required`とする。`required`時は、repo instructions指定remote、またはtarget common Git directoryに属する全worktreeで一意なhosted identityを持つremote URLだけを使う。HTTPS、`ssh://`、scp-like SSH URLをcredentialや末尾の`.git`に依存しないcanonical host / owner / repositoryへ正規化し、local common Git directoryとbindingする。basename、remote名、現在branch、最初に見つかったremoteから推測しない。`required`なのにremoteなし、複数の異なるhosted identity、URL parse不能、bindingの矛盾またはsnapshot間の変化がある場合、そのselector evidenceを持つtaskを`unscoped`としてfail closedにする。local-only evidenceまでhosted binding不在だけでblockしない。snapshotにはhosted binding requirement stateと、`required`時だけbinding値を含める。
...
- **routing**: 明示されたtool `cwd` / `workdir`をtaskのcanonical cwdから解決し、なければtask cwdをeffective baseとする。tool schemaのpath field、patch header、literalなfile operand、shellの順序付きliteral `cd`、shell redirect target、およびGit invocationのordered repository selectorを抽出する。Gitでは`-C`、`--git-dir=<path>` / `--git-dir <path>`、`--work-tree=<path>` / `--work-tree <path>`、command-scoped `GIT_DIR` / `GIT_WORK_TREE` / `GIT_COMMON_DIR`を実際のoption / environment semanticsに従って解決し、repository routingを変える全pathを検査する。relative valueはそのselector評価時点のeffective baseから解決する。selectorの重複・競合、動的値、environment継承、option termination、subcommand境界を一意に解釈できなければ`unscoped`とする。output / append / read-write redirectはmutation、input redirectはread routingとしてeffectへ渡す。tool schemaのrepository selector（`owner` + `repo`、`repo_full_name`、`repository_full_name`、repository URL / identity、およびschemaが同義と定義するfield）はhosted bindingを`required`にし、実値をcanonical host / owner / repository identityへ正規化してauthoritative hosted identityと比較する。hostを省略するselectorはtool schemaが一意なhostを保証する場合だけ補完する。target hosted identityのbinding、alias、default、欠落、矛盾を一意に解決できなければ当該taskを`unscoped`とする。`.` / `..`とsymlinkを解決し、存在しない末尾は最長の存在ancestorをcanonicalizeしてからpath separator境界を保って正規化する。各解決済みpathは、directoryなら自身、fileならparent、存在しない末尾なら最長の存在ancestorを起点にrepository-identity-only probeを行う。canonical common Git directoryがtarget local identityと一致するか、Git snapshot取得後の全target worktree canonical rootのいずれかとpath separator境界付きで一致する場合は、単一target root外のlinked worktreeでもtarget-relevantとする。probeが成功して別common Git directoryを返し、target worktree rootsとの境界付き包含もない場合はtarget-irrelevantとする。canonical pathの存在・access確認に成功し、安定化したlocaleで明確な`not a git repository`が返り、ancestorに`.git` markerがなく、target worktree rootsとの境界付き包含もない場合もtarget-irrelevantとする。permission、I/O、timeout、parse、dubious ownership、壊れた`.git`、non-Git判定不能、target worktree集合との矛盾、またはidentity / worktree集合のsnapshot間変化は`unscoped`とする。文字列prefix、basename、target root外という理由だけで別repoと判定しない。`cwd` / `workdir` key、repository selector key、redirect token、またはGit option tokenの存在だけではtarget-relevantにしない。opaque executable、local script、package / task runnerは、schemaまたはinspectionしたsource / manifest / resolved command chainがpath confinementを保証する場合だけ全operandを別repositoryと確定できる。動的download、生成path、変数、`eval`、unknown binary、partial inspection、またはrouting semanticsを完全に確認できない実行は`unscoped`としてfail closedにする。
- **provenance**: defaultは`task-owned`とする。guard開始時、最初のtool callより前にcurrent interactionのimmutable origin item boundary、versioned outer run ID、attempt `1`、append-only ledgerを固定する。この時点でcaller task ID / `hostId`がruntimeのtrusted metadataから得られなければ`null`のprovisional callerとし、取得のためだけにtool callを先行させない。各許可済みevidence collection invocationは発行直前に、outer run ID、attempt、origin / attempt boundary、単調増加ordinal、purpose、tool名、canonical arguments SHA-256、expected routing / effect / lifecycleからdeterministic `callIdentity`を固定してledgerへ追加する。caller解決後は、full history上でorigin boundaryと全call sequenceを含むtaskがexactに1件であることを検証してtask ID / `hostId`をledgerへbindingする。0件、複数、partial、またはtrusted metadataとの不一致なら`unscoped`とする。

  Workflow 1–9で発行したinvocationは、観測後のeffect / lifecycle、attempt boundaryもledgerへ追記し、各read passで一意に対応したexact item IDを`guard-owned`とする。内部retryではouter run ID、origin boundary、ledgerを維持し、attempt番号とattempt boundaryだけを追加する。retry前のentryを削除・初期化しない。対象はrepo root / identity、script path lookup、task inventory / scope read、preflight、fallback snapshot、post-read identity probeに限定する。mutationを許すのは、authoritative baselineをfreshにするためWorkflowで明示したbounded ref refreshだけとし、terminal完了、固定したref / SHA、観測したeffectが契約どおりであることをGit snapshotで検証する。その他のmutation、unfinished / unknown lifecycle、想定外effectはfail closedにする。同じcommand文字列という理由だけで除外しない。対応が欠落、重複、partial、または一意でなければfail closedにする。`guard-owned` itemはscope anchor / fingerprintへ入れず、Git / task / identity evidenceとして別管理する。

  完了・停止を問わず各outer guard runの結果を返す直前に、stableなassistant message itemとして`guard provenance record`を1個残す。recordは`payload`と`recordHash`をsibling fieldとして持つ。`payload`はschema version、outer run ID、origin boundary、caller task ID / `hostId`または`null`、全attempt boundary、target local identity、hosted binding requirement state / optional identity、terminal result、`complete`を持ち、ledger各項目を全attempt通算の発行順で`callIdentity`、attempt、exact item IDまたは`null`、purpose、tool名、canonical argumentsのSHA-256、解決済みまたはexpected routing / effect / lifecycleで表す。caller未解決、history readのpartial / unavailable / truncated、またはitem対応前に停止する場合は`complete: false`とし、発行済みattemptを`itemId: null`のまま欠落させず記録する。callerが`null`のrecordは、後続runが完全なhistoryからorigin boundaryと全call sequenceを含むtaskをexactに1件だけ特定し、そのtask ID / `hostId`へbindingできた場合だけ補完できる。`recordHash`は自身を含めず`payload`だけをRFC 8785 JSON Canonicalization SchemeでserializeしたUTF-8 bytes（BOMなし、前後whitespace / trailing newlineなし）のSHA-256 lowercase hexadecimalとする。duplicate key、non-finite number、RFC 8785で表現不能な値はrecord作成・検証をfail closedにする。
...
1. 最初のtool callより前にimmutable origin item boundary、versioned outer run ID、attempt `1`、provisional caller、空のappend-only guard-owned ledgerを固定する。各tool callの発行直前にdeterministic `callIdentity` entryを追加する。repo rootとlocal repository identityだけを解決し、発行したinvocationをpurpose付きでledgerへ追記する。hosted identityはまだ要求せず、structured selectorを観測した場合だけ同じrun内で遅延解決する。この時点ではbranch、worktree一覧、HEAD、dirty state、diffを個別Git commandで収集しない。
...
9. preflightまたはfallback Git snapshot後、次の順でpost-preflight snapshotを取得する。
   1. hosted bindingが`required`の場合だけ同じ規則で再解決し、initialと同じlimitの`list_threads`とrepository identityの対応付けからinventory Aを作り、coverageを記録する。guardが発行した各repository identity probeをpurpose付きでguard-owned ledgerへ追加する。
